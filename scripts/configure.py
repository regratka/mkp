...
def configure(config_dir: Path, build_dir: Path, include_dir: Path, src_dir: Path, importFile: Path):
    units = []
    with (config_dir/"units_listing.csv").open("r") as f:
        for line in f:
    ...
    generateSources(str(config_dir/"mapping.csv"), 
                    str(config_dir/"units_listing.csv"),
                    src_dir, include_dir)
    generateNinja(units, build_dir/"src", src_dir, include_dir, config_dir.parent.parent)
    generateObjdiffConfig(build_dir/"orig", build_dir/"src", units, config_dir.parent.parent)
